 ...
 * @return array
 */
function ironmasslite_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = ironmasslite_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = ironmasslite_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = ironmasslite_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? ironmasslite_post_thumbnail( 'ironmasslite-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? ironmasslite_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? ironmasslite_posted_on( array( 'echo' => false ) ) : '';
